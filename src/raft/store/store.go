...
func (s *Store) Open(joinAddr, path, bindAddr string) error {
	...
	// NewRaft Method Documentation: http://godoc.org/github.com/hashicorp/raft#NewRaft
	// NewRaft(conf *Config, fsm FSM, logs LogStore, stable StableStore, snaps SnapshotStore, peerStore PeerStore, trans Transport) (*Raft, error)

	ra, err := raft.NewRaft(config, s.fsm, logStore,
		logStore, snapshots, peerStore, transport)
...
// AddPeer is used to add a new peer into the cluster. This must be run
// on the leader or it will fail.
...
// Leader is used to return the current leader of the cluster. It may
// return empty string if there is no current leader or the leader is
// unknown.
...
// Note: This happens outside of raft, but we could use raft to actually
// return the current state if we wanted to
...
func (s *Store) Set(key, value string) error {
	...
	// This is likely an error, type assert to find out. You must wait for the
	// error above to return first before checking this value.
	if f.Response() != nil {
